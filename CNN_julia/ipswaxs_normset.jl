...
# System Parameters
currdir = pwd();
datadir = "F:\\Yen\\ShapeData_Normalized";
cd(datadir);
...
ipf = InnerProductLayer(name="ipf", output_dim=dimout, neuron=Neurons.ReLU(), bottoms=[:data], tops=[:ipf]);
sm = SoftmaxLayer(name="sm", bottoms=[:ipf], tops=[:sm]);
loss = BinaryCrossEntropyLossLayer(name="loss", bottoms=[:sm, :label]);

common_layers = [ipf; sm];
...
accuracy = BinaryCrossEntropyLossLayer(name="validation-accuracy", bottoms=[:sm, :label]);
...
# Training parameters
exp_dir = "snapshots-ipswaxs-normset-0328-pc-$(Mocha.default_backend_type)";
maxiter = 100000;
learning_rate = 1e-5;
...
# Coffee break
summary_interval = 10;
snapshot_interval = Int64(maxiter/400);
